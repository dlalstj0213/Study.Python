...
class QStackedWidget(QtWidgets.QStackedWidget):
    ...
    def setFadeTransition(self, fadeState):
        if isinstance(fadeState, bool):
            ...
            raise Exception(
                "setFadeTransition() only accepts boolean variables")
    ...
    def setSlideTransition(self, slideState):
        if isinstance(slideState, bool):
            ...
            raise Exception(
                "setSlideTransition() only accepts boolean variables")
    ...
    def slideToWidget(self, newWidget):
        ...
        if self.fadeTransition:
            FadeWidgetTransition(self, self.widget(
                _currentWidgetIndex), self.widget(_nextWidgetIndex))
    ...
    def setCurrentWidget(self, widget):
        ...
        if self.fadeTransition:
            self.fader_widget = FadeWidgetTransition(self, self.widget(
                self.currentIndex()), self.widget(self.indexOf(widget)))
                                    ...
                                    for fade in transitionAnimation["fade"]:
                                        if "active" in fade and fade["active"]:
                                            ...
                                            if "easingCurve" in fade and len(str(fade["easingCurve"])) > 0:
                                                widget.fadeEasingCurve = returnAnimationEasingCurve(
                                                    fade["easingCurve"])
                                    ...
                                    for slide in transitionAnimation["slide"]:
                                        if "active" in slide and slide["active"]:
                                            ...
                                            if "direction" in slide and len(str(slide["direction"])) > 0:
                                                widget.transitionDirection = returnQtDirection(
                                                    slide["direction"])
                            ...
                            for navigation in stackedWidget["navigation"]:
                                if "nextPage" in navigation:
                                    if hasattr(self, str(navigation["nextPage"])):
                                        button = getattr(
                                            self, str(navigation["nextPage"]))
                                        button.clicked.connect(
                                            lambda: widget.slideToNextWidget())
                                    else:
                                ...
                                if "previousPage" in navigation:
                                    if hasattr(self, str(navigation["previousPage"])):
                                        button = getattr(
                                            self, str(navigation["previousPage"]))
                                        button.clicked.connect(
                                            lambda: widget.slideToPreviousWidget())
                                    else:
                                        print("No button found")
                                    ...
                                    for navigationButton in navigation["navigationButtons"]:
                                        for button in navigationButton:
                                            ...
                                            if not hasattr(self, str(widgetPage)):
                                                raise Exception(
                                                    "Unknown widget '" + str(widgetPage) + "'. Please check your JSon file")
                                            ...
                                            widgetPg = getattr(
                                                self, str(widgetPage))
                                            navigationButtons(
                                                widget, pushBtn, widgetPg)


def navigationButtons(stackedWidget, pushButton, widgetPage):
    pushButton.clicked.connect(
        lambda: stackedWidget.setCurrentWidget(widgetPage))
...
def returnAnimationEasingCurve(easingCurveName):
    if len(str(easingCurveName)) > 0:
        if str(easingCurveName) == "OutQuad":
            ...
            raise Exception("Unknown value'" + easingCurveName +
                            "' for setEasingCurve() on ", animation)
...
def returnQtDirection(direction):
    if len(str(direction)) > 0:
        if str(direction) == "horizontal":
            ...
            raise Exception("Unknown direction name given ("+direction +
                            "), please use Vertical or Horizontal direction")

    else:
        raise Exception(
            "Empty direction name given, please use Vertical or Horizontal direction")
